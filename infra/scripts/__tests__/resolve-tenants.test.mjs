...
describe('tenant registry discovery', () => {
  test('loads tenant metadata from the repository', () => {
    ...
    expect(gary.secretFileName).toBe('GARYGERBER_COM_VITE_ENV-secrets')
    expect(Array.isArray(gary.envSecretKeys)).toBe(true)
    expect(gary.envSecretKeys.length).toBeGreaterThan(0)
    expect(gary.displayName).toContain('Gary')
    expect(gary.supportEmail).toBe('support@garygerber.com')
