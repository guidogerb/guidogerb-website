...
export default function AuthProvider({
  ...
  const cfg = {
    ...
    authority:
      norm(authority) ?? norm(env?.VITE_COGNITO_AUTHORITY) ?? norm(env?.VITE_COGNITO_DOMAIN),
  ...
  if (missing.length > 0) {
    ...
    console.error(
      '[AuthProvider] Missing required OIDC settings:',
      missing.join(', '),
      '\nConfig received:',
      cfg,
    )
      ...
      <div style={{ border: '1px solid #ccc', padding: 12, marginTop: 16 }}>
        ...
        <div>
          ...
          <div style={{ marginTop: 8, color: '#a00' }}>
            Hint: ensure OIDC is configured via environment or props. For production, set VITE_ENV
            secret to include VITE_COGNITO_AUTHORITY or VITE_COGNITO_METADATA_URL,
            VITE_COGNITO_CLIENT_ID, and VITE_REDIRECT_URI.
          </div>
        </div>
      </div>
    )
  ...
  return (
    <OidcAuthProvider {...cfg}>{isLoginCallback ? <LoginCallback /> : children}</OidcAuthProvider>
  )
}
