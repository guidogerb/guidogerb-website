 ...
 * @typedef {'wordpress' | 'static site' | 'salesforce' | 'custom application' | 'microsoft power apps'} HeaderApplicationTypes
 */
...
/**
 * IdFetchStyle
 * @typedef {'Automatic' | 'None' | 'Provided'} IdFetchStyle
 */
 ...
 *  @property {() => boolean} [isPerformPopup] should the popup pop open? Helpful for ggpId that doesn't pop until user loaded
 ...
 *  @property {'left' | 'none' | 'right'} [mobileMenuLocation] - positioned right or left of the ggp ID button? not at all? default is none
...
/**
 * // User fields from the GgpId authority
 * @typedef UserInfo {
 ...
 *  @property {string | undefined} [env] - the GgpId environment
 *  @property {string | null | undefined} first - the name shown on the GgpId button when logged in
 ...
 * // The GgpID header will auto fetch the user from GgpID if it is not told that your application will be controlling the signed in user
 * // This may cause the header to jump as data comes from ggpID and your application gets data from its data source.
 ...
 * @typedef GgpIDSettings {
 *  @property {UserInfo | undefined | null} currentUser - null: app controls the user, undefined: header will fetch current user
 *  @property {function(GgpIdData): void | undefined} [onAuthChanged] - auth user changes, eg (newUserData) => { ... do something ... }
 *  @property {function(UIEvent): void | undefined} [onProfile] - when the GgpId's menu item for the user's profile is triggered: (e) => { }
 *  @property {function(UIEvent): void | undefined} [onSignIn] - when the GgpId button is pressed to sign in: (e) => { }
 *  @property {function(UIEvent): void | undefined} [onSignOut] - when the ggpId's menu item for sign out is triggered: (e) => { }
 *  @property {MenuItem[] | undefined} [menuItems] - menu items to add to the ggpId menu (user must be logged in to open the menu): (e) => { }
 * }
 */
 ...
 *  @property {UtahIDSettings | boolean} [ggpId] - settings for the ggpId button; true = turned on, false = turned off, object = custom
