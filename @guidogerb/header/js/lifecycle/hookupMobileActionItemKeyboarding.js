...
export function hookupMobileActionItemKeyboarding() {
  ...
  allActionItemButtons.forEach((draftActionItemButton) => {
    ...
    draftActionItemButton.onkeyup =
      ...
      (e) => {
        ...
        if (e.code === 'Home') {
          ...
          newIndex =
            (allActionItemButtons.length + actionItemButtonIndex - 1) % allActionItemButtons.length
        }
        if (!Number.isNaN(newIndex)) {
          ...
          const newWrapper = /** @type {HTMLElement} */ (
            newActionItemButton?.closest(
              getCssClassSelector(domConstants.MOBILE_MENU_ACTION_BAR__ACTION_ITEM_WRAPPER),
            )
          )
          if (!newWrapper) {
            throw new Error(
              'hookupMobileActionItemKeyboarding: newWrapper not found... how in the blazes?!',
            )
          }

          const newActionItemContentId = newActionItemButton?.getAttribute('aria-controls')
          const newActionItemContent =
            newActionItemContentId && document.getElementById(newActionItemContentId)
          ...
          if (newActionItemContent) {
            ...
            const mobileContentWrapper = /** @type {HTMLElement} */ (
              mobileMenuWrapper.querySelector(
