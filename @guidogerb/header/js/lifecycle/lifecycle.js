...
import {
  hookupIdInMobileMenu,
  removeIdInMobileMenu,
} from '../renderables/mobile/hookupggpIdInMobileMenu'
...
import { renderOfficialWebsite } from '../renderables/ggpLogo/renderOfficialWebsite'
import { getHeaderSettings } from '../settings/getHeaderSettings.js'
import { fetchIdUserDataAsync } from '../ggpId/ggpIdData'
...
function loadCssSettings() {
  ...
  const mediaQueriesCssReplaced = mediaQueriesCSS
    .replace(
      domConstants.MEDIA_SIZE__TABLET_PORTRAIT__PLACEHOLDER,
      `${getHeaderSettings().mediaSizes.tabletPortrait}px`,
    )
    .replace(
      domConstants.MEDIA_SIZE__TABLET_LANDSCAPE__PLACEHOLDER,
      `${getHeaderSettings().mediaSizes.tabletLandscape}px`,
    )
    .replace(
      domConstants.MEDIA_SIZE__MOBILE__PLACEHOLDER,
      `${getHeaderSettings().mediaSizes.mobile}px`,
    )
...
/**
 * Based on the current settings, determine in to which DOM element to insert the  ui system header.
...
function determineTargetElementForHeader() {
  ...
  if (settings.domLocationTarget) {
    ...
    if (targets.length < 1) {
      throw new Error(
        'loadHeader: domLocationTarget must either have a value for one of its properties or not be specified at all',
      )
    ...
    if (settings.domLocationTarget.cssSelector) {
      ...
      checkForError(
        !domTarget,
        `loadHeader: element not found for domLocationTarget.cssSelector ${settings.domLocationTarget.cssSelector}`,
      )
      ...
      checkForError(
        !domTarget,
        'loadHeader: element not returned from domLocationTarget.elementFunction',
      )
    } else {
...
export function loadHeader() {
  const existingHeader = document.querySelector(
    getCssClassSelector([domConstants.UI_SYSTEM, domConstants.HEADER]),
  )
  if (!existingHeader) {
    ...
    // load the main menu
    const { mainMenuWrapper, ggpIdPopup } = renderMainMenu()
    if (mainMenuWrapper) {
    ...
    const mobileMenuBackdrop = /** @type {HTMLElement} */ (
      document.querySelector(getCssClassSelector(domConstants.MOBILE_MENU__BACKDROP))
    )
    ...
    if (ggpIdPopup) {
      ...
      ggpIdPopup.closest('div')?.removeAttribute('aria-labelledby')
      hookupGgpIdInMobileMenu(mobileMenuWrapper, ggpIdPopup)
    } else {
      removeGgpIdInMobileMenu()
    }
    ...
    // UDS-564
    // there are four parts to deciding the state of the main menu bar: main menu, search, ggpId, action items
    ...
    if (
      ...
      (!settings.mainMenu &&
        ...
        settings.ggpId === false &&
        settings.onSearch) ||
      (!settings.mainMenu && settings.actionItems && settings.ggpId === false && !settings.onSearch)
    ) {
      mobileMenuWrapper.classList.add(domConstants.MAIN_MENU__REMOVED)
      mainMenuWrapper.remove()
    }
    if (
      (!settings.mainMenu && !settings.actionItems && settings.ggpId && !settings.onSearch) ||
      (!settings.mainMenu && settings.actionItems && settings.ggpId && !settings.onSearch) ||
      (!settings.mainMenu && settings.actionItems && !settings.ggpId && settings.onSearch)
    ) {
...
export function removeHeader(shouldTriggerUnloadEvent) {
  document
    .querySelector(getCssClassSelector([domConstants.UI_SYSTEM, domConstants.SKIP_LINK_WRAPPER]))
    ?.remove()
  document
    .querySelector(getCssClassSelector([domConstants.UI_SYSTEM, domConstants.HEADER]))
    ?.remove()
  document
    .querySelector(getCssClassSelector([domConstants.UI_SYSTEM, domConstants.MAIN_MENU__OUTER]))
    ?.remove()
  document
    .querySelector(getCssClassSelector([domConstants.UI_SYSTEM, domConstants.MOBILE_MENU]))
    ?.remove()
  document
    .querySelector(
      getCssClassSelector([domConstants.UI_SYSTEM, domConstants.LOGO_OFFICIAL_WRAPPER]),
    )
    ?.remove()
