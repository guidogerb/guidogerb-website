...
export function showActionItem(mobileMenuWrapper, actionItemWrapper) {
  ...
  mobileMenuWrapper
    .querySelectorAll(getCssClassSelector(domConstants.MOBILE_MENU_ACTION_BAR__ACTION_ITEM_WRAPPER))
    .forEach((actionItem) => {
...
export function showContentItem(mobileContentWrapper, mobileMenuContentItem) {
  ...
  mobileContentWrapper
    .querySelectorAll(getCssClassSelector(domConstants.MOBILE_MENU__CONTENT_ITEM))
 ...
 * @param {(e: MouseEvent) => boolean} [options.onClickHandler] - returns true: no further action will be taken (for GgpID), false: behave normally
...
export function mobileMenuInteractionHandler(
  ...
  actionItemWrapper,
  { ariaHasPopupType, additionalOnClick, onClickHandler, shouldOnClickCloseMenu },
) {
  if (mobileMenuContentItem && !ariaHasPopupType) {
    throw new Error(
      'mobileMenuInteractionHandler: there is content, but the aria type is not given',
    )
  }
  const mobileMenu = /** @type {HTMLElement} */ (
  ...
  const mobileMenuWrapper = /** @type {HTMLElement} */ (
    document.querySelector(getCssClassSelector(domConstants.MOBILE_MENU__WRAPPER))
  )
  ...
  if (mobileMenuContentItem) {
    ...
    if (mobileMenuContentItemValue) {
      ...
      ;(actionItemButton || interactiveElement).setAttribute(
        'aria-controls',
        mobileMenuContentItemId,
      )
  ...
  // setup opening closing for things like ggpid/hamburger
