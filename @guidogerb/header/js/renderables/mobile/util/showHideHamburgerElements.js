...
export function hideMobileMenu() {
  ...
  // change from an 'X'
  hamburgerIcon?.classList.add('icon-before-hamburger')
  hamburgerIcon?.classList.remove('icon-before-x-icon')
...
export function showMobileMenu() {
  ...
  if (mainMenuWrapper?.classList.contains(domConstants.MOBILE__HIDDEN)) {
    document
      .querySelector(
        `${getCssClassSelector(domConstants.UI_SYSTEM)}${getCssClassSelector(domConstants.MOBILE_MENU)}`,
      )
      ?.classList.add(domConstants.MAIN_MENU__REMOVED)
  }
  ...
  // change to an 'X'
  hamburgerIcon?.classList.remove('icon-before-hamburger')
  hamburgerIcon?.classList.add('icon-before-x-icon')
