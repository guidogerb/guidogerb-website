...
import { closeOfficialWebsite } from '../ggpLogo/renderOfficialWebsite'
...
export function hookupHamburger(mobileMainMenuContentItem) {
  ...
  function firstBarActionItem() {
    const actionItem = notNull(
      /** @type {HTMLElement} */ (
        document.querySelectorAll(
          `.${domConstants.MOBILE_MENU__ACTION_BAR} .${domConstants.MOBILE_MENU_ACTION_BAR__ACTION_ITEM_WRAPPER}`,
        )[0]
      ),
    ...
    return {
      actionItem,
      actionItemWrapper: contentWrapperId ? document.getElementById(contentWrapperId) : null,
    }
  }
  ...
  if (hamburger) {
    ...
    hamburger.onblur = () => {
      ...
      if (mobileMenu.classList.contains(domConstants.IS_OPEN)) {
        ...
        const actionItemWrapperClass = getCssClassSelector(
        ...
        const firstMobileActionItem =
          ...
          (
            document.querySelector(
              `${actionBarClass} ${actionItemWrapperClass}:first-child ${actionItemButtonClass}`,
            )
          )
  ...
  if (
    (settings.mainMenu || settings.actionItems) &&
    !settings.onSearch &&
