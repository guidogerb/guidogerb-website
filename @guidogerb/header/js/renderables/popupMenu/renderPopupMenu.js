...
function toggleChildMenuExpansion(element) {
  ...
  parent.querySelectorAll(':scope > ul').forEach((childUl) => {
    // toggle chevron direction class
    const button = childUl
      .closest('li')
      ?.querySelector(getCssClassSelector(domConstants.VERTICAL_MENU__BUTTON_TITLE))
    if (!button) {
      throw new Error('toggleChildMenuExpansion: button not found')
    }
    const chevron = button.querySelector(
      `:scope > ${getCssClassSelector(domConstants.VERTICAL_MENU__CHEVRON)}`,
    )
    if (!chevron) {
...
function renderPopupMenuItem(menuUl, popupMenuItem, options) {
  ...
  const menuItemTitleWrapper = menuItemWrapper.querySelector(
    getCssClassSelector(domConstants.VERTICAL_MENU__TITLE),
  )
  ...
  const menuButton = /** @type {HTMLElement | null} */ (
    menuItemWrapper.querySelector(getCssClassSelector(domConstants.VERTICAL_MENU__BUTTON_TITLE))
  )
  if (!menuButton) {
    throw new Error('renderPopupMenuItem: menuButton not found')
  }
  const menuAHref = /** @type {HTMLElement | null} */ (
    menuItemWrapper.querySelector(getCssClassSelector(domConstants.VERTICAL_MENU__LINK_TITLE))
  )
  if (!menuAHref) {
    throw new Error('renderPopupMenuItem: aHref not found')
  }
  const plainTitle = /** @type {HTMLElement | null} */ (
    menuItemWrapper.querySelector(getCssClassSelector(domConstants.VERTICAL_MENU__PLAIN_TITLE))
  )
  if (!plainTitle) {
    throw new Error('renderPopupMenuItem: plainTitle not found')
  }
  const menuDivider = menuItemWrapper.querySelector(
    getCssClassSelector(domConstants.VERTICAL_MENU__DIVIDER),
  )
  if (!menuDivider) {
    throw new Error('renderPopupMenuItem: menuDivider not found')
  }
  const titleSpanButton = menuItemWrapper.querySelector(
    `button ${getCssClassSelector(domConstants.VERTICAL_MENU__LINK_TEXT)}`,
  )
  if (!titleSpanButton) {
    throw new Error('renderPopupMenuItem: titleSpanButton not found')
  }
  const titleSpanLink = menuItemWrapper.querySelector(
    `a ${getCssClassSelector(domConstants.VERTICAL_MENU__LINK_TEXT)}`,
  )
  if (!titleSpanLink) {
  ...
  if (
    actionMenu &&
    (popupMenuItem.actionFunction || popupMenuItem.actionUrl || popupMenuItem.actionFunctionUrl) &&
  ...
  if (actionMenu) {
    ...
    switch (options.childrenMenuType) {
      case childrenMenuTypes.FLYOUT: {
        ...
        popupFocusHandler(menuItemWrapper, menuButton, subMenuItemsPopup, 'menu', {
      ...
      case childrenMenuTypes.INLINE: {
        const subMenu = renderMenu(actionMenu, options)
        ...
        menuItemWrapper.addEventListener('focusin', (e) => {
          ...
          for (
            let childUl = /** @type {Element | null | undefined} */ (e.target)?.closest('ul');
            childUl;
            childUl = childUl.parentElement?.closest('ul')
          ) {
      ...
      default:
        throw new Error(
          `renderPopupMenuItem: childrenMenuType unknown '${options.childrenMenuType}'`,
        )
    }
    menuDivider.remove()
    ...
    throw new Error(
      `renderPopupMenuItem: popupMenuItem must have either actionMenu, actionFunction, actionUrl, or isDivider (${'popupMenuItem.title'})`,
    )
  }
  ...
  const selectedClassName = popupMenuItem.actionMenu?.length
    ? domConstants.MENU_ITEM__SELECTED_PARENT
    : domConstants.MENU_ITEM__SELECTED
  // add selected to title if selected (or any children are selected)
  if (
    popupMenuItem.isSelected ||
    (popupMenuItem.actionMenu &&
      findRecursive(
        popupMenuItem.actionMenu,
        ['actionMenu'],
        (checkMenuItem) => !!checkMenuItem.isSelected,
      ))
  ) {
...
export function renderPopupMenu(popupMenu, labelledByElement, options) {
  ...
  const popupWrapper = renderPopup(labelledByElement, {
    removePopupArrow: options.removePopupArrow,
  })

  // put the menu in the popup
  const popupContentWrapper = popupWrapper.querySelector(
    getCssClassSelector(domConstants.POPUP_CONTENT_WRAPPER),
  )
