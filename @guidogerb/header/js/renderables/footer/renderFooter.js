...
export function renderFooter() {
  ...
  if (settings.footer === null) {
    ...
    const settingsChanged =
      previousFooterSettings.domLocationTarget.cssSelector !==
        settings?.footer?.domLocationTarget?.cssSelector ||
      previousFooterSettings.domLocationTarget.element !==
        settings?.footer?.domLocationTarget?.element ||
      previousFooterSettings.domLocationTarget.elementFunction !==
    ...
    if (settingsChanged || !previousFooter) {
      ...
      if (settings?.footer?.domLocationTarget) {
        ...
        checkForError(
          targets.length < 1,
          'renderFooter: footer.domLocationTarget must either have a value for one of its properties or not be specified at all',
        )
        checkForError(
          targets.length > 1,
          'renderFooter: footer.domLocationTarget must only have one target specified',
        )
        ...
        if (settings?.footer?.domLocationTarget.cssSelector) {
          ...
          checkForError(
            !domTarget,
            `renderFooter: element not found for domLocationTarget.cssSelector ${settings?.footer?.domLocationTarget.cssSelector}`,
          )
          ...
          checkForError(
            !domTarget,
            'renderFooter: element not returned from domLocationTarget.elementFunction',
          )
        } else {
      ...
      if (!settings?.footer?.showHorizontalRule) {
        const footerHr = document.querySelector(
          getCssClassSelector(domConstants.FOOTER_HORIZONTAL_DIVIDER),
        )
      ...
      const footerLinks = notNull(
        document.querySelector(getCssClassSelector(domConstants.FOOTER_LINKS)),
        'renderFooter: footer links not found',
      )
