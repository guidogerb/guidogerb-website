...
function searchModalElements(searchModal) {
  const searchModalUse = notNull(
    searchModal || document.getElementById(domConstants.SEARCH__SEARCH_MODAL),
    `setupSearchModal: '#${domConstants.SEARCH__SEARCH_MODAL}' not found`,
  )

  const backdrop = /** @type {HTMLElement} */ (
    notNull(
      searchModalUse.querySelector(getCssClassSelector(domConstants.SEARCH__SEARCH_BACKDROP)),
      'setupSearchModal: backdrop not found',
    )
  )

  const closeButton = /** @type {HTMLElement} */ (
    notNull(
      searchModalUse.querySelector(getCssClassSelector(domConstants.SEARCH__SEARCH_CLOSE_BUTTON)),
      'setupSearchModal: closeButton not found',
    )
  )

  const searchButton = /** @type {HTMLElement} */ (
    notNull(
      searchModalUse.querySelector(getCssClassSelector(domConstants.SEARCH__SEARCH_BUTTON)),
      'setupSearchModal: searchButton not found',
    )
  )

  const searchButtonWrapper = /** @type {HTMLElement} */ (
    notNull(
      searchModalUse.querySelector(getCssClassSelector(domConstants.SEARCH__SEARCH_BUTTON_WRAPPER)),
      'setupSearchModal: searchButtonWrapper not found',
    )
  )

  const searchInput = /** @type {HTMLInputElement} */ (
    notNull(
      searchModalUse.querySelector(getCssClassSelector(domConstants.SEARCH__SEARCH_INPUT)),
      'setupSearchModal: searchInput not found',
    )
  )

  const hiddenLastFocusableButton = /** @type {HTMLElement} */ (
    notNull(
      searchModalUse.querySelector('.search-modal__hidden-last-focusable'),
      'setupSearchModal: hiddenLastFocusableButton not found',
    )
  )
...
function closeSearchModal() {
  const searchWrapper = notNull(
    document.getElementById(domConstants.SEARCH__SEARCH_MODAL),
    'closeSearchModal: search modal not found',
  )
  searchWrapper.classList.add(domConstants.VISUALLY_HIDDEN)

  const { closeButton, hiddenLastFocusableButton } = searchModalElements()

  const searchIcon = /** @type {HTMLElement} */ (
    document.querySelector(getCssClassSelector(domConstants.MAIN_MENU__SEARCH))
  )
...
export function showSearchModal() {
  const searchWrapper = notNull(
    document.getElementById(domConstants.SEARCH__SEARCH_MODAL),
    'showSearchModal: search modal not found',
  )
