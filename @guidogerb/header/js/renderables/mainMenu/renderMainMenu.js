...
import { renderGgpIdForMobile } from '../ggpId/GgpId'
...
/**
 * @returns {{mainMenuWrapper: HTMLElement, ggpIdPopup: HTMLElement | null}}
 */
export function renderMainMenu() {
  ...
  if (settings.mainMenu) {
    const mainMenuTop = mainMenuNav?.querySelector(
      getCssClassSelector(domConstants.MAIN_MENU__MENU_TOP),
    )
    if (!mainMenuTop) {
    ...
    settings.mainMenu.menuItems.forEach((menuItem) => {
      ...
      mainMenuItemButtonTitle.setAttribute(
        'id',
        `${domConstants.MENU_ITEM__BUTTON_TITLE}__${menuItem.title}-${uuidv4()}`,
      )
      ...
      mainMenuItemLinkTitle.setAttribute(
        'id',
        `${domConstants.MENU_ITEM__LINK_TITLE}__${menuItem.title}-${uuidv4()}`,
      )
      if (menuItem.className) {
      ...
      const menuItemTitleSpanElement = notNull(
        menuItemTitleElement.querySelector(
          getCssClassSelector(domConstants.MENU_ITEM__LINK_TITLE_SPAN),
        ),
      ...
      if (menuItem.actionMenu) {
        ...
        if (
          (menuItem.actionFunction || menuItem.actionUrl || menuItem.actionFunctionUrl) &&
        ...
        const subMenuPopup = renderPopupMenu(popupMenu, menuItemTitleElement, {
        ...
        popupFocusHandler(mainMenuItem, menuItemTitleElement, subMenuPopup, 'menu', {
  ...
  let ggpIdPopup = null
  if (settings.ggpId) {
    const { button: ggpIdButton, menu } = renderGgpIdForMobile()
    ggpIdPopup = menu
    const ggpIdButtonWrapper = notNull(
      mainMenuWrapper.querySelector(getCssClassSelector(domConstants.MOBILE__UTAH_ID)),
      'renderMainMenu: ggpIdButtonWrapper not found',
    )
    ggpIdButtonWrapper.appendChild(ggpIdButton)
  }
  ...
  if (settings.onSearch) {
    ...
    if (!settings.mainMenu) {
      ...
      // if actions AND search, but not ggpId nor menu
      ...
      // search icon is the far right item in the citizen experience (no ggpid button)
      // there is no main menu, so move search in to top header by ggpID button UDS-564
      if (settings.ggpId !== false) {
        // place search icon just to left of ggpId button
        const ggpIdWrapper = notNull(
          document.querySelector(getCssClassSelector(domConstants.UTAH_ID)),
          'renderMainMenu: ggpId wrapper not found',
        )
        citizenExperienceWrapper.insertBefore(mobileSearchIcon, ggpIdWrapper)
      } else {
    ...
    if (!settings.mainMenu && !settings.actionItems && settings.ggpId === false) {
      // create search clone
  ...
  if (!settings.onSearch) {
    if (settings.mainMenu || (!settings.mainMenu && settings.ggpId)) {
      // add a blank div to consume space (only if there is a main menu)
      searchIcon.parentElement?.insertBefore(
        renderDOMSingle('<div class="main-menu__search-placeholder">'),
        searchIcon,
      )
    }
    searchIcon.remove()
  ...
  return { mainMenuWrapper, ggpIdPopup }
}
