...
export function renderMobileActionItems() {
  ...
  const hasGgpId = !!getHeaderSettings().ggpId
  const profileActionItem = hasGgpId
    ? document.getElementById(domConstants.MOBILE_MENU_ACTON_BAR__PROFILE_ID)
    : null
  ...
  ;[...(getHeaderSettings().actionItems || [])]
    ?.reverse()
    ?.map((actionItemToRender) => ({
      actionItem: actionItemToRender,
      ...renderMobileActionItem(actionItemToRender),
    }))
    ?.forEach(({ actionItem, actionItemContent, actionItemElement }) => {
      ...
      if (actionItemsWrapper) {
        ...
        if (actionItemContent) {
          ...
          mobileMenuInteractionHandler(actionItemElement, mobileContentItem, actionItemElement, {
            ariaHasPopupType,
            shouldOnClickCloseMenu: false,
          })
        }
      }
    })
