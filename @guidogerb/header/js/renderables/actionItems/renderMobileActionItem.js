...
export function renderMobileActionItem(actionItem) {
  ...
  const actionItemWrapper = /** @type {HTMLElement} */ (
    actionItemElement instanceof HTMLCollection ? actionItemElement[0] : actionItemElement
  )
  ...
  const titleDiv = actionItemWrapper.querySelector(
    getCssClassSelector(domConstants.ACTION_ITEM__TITLE),
  )
  if (!titleDiv) {
  ...
  const iconButton = /** @type {HTMLElement} */ (
    actionItemElement.querySelector(getCssClassSelector(domConstants.ACTION_ITEM__ICON_BUTTON))
  )
  if (!iconButton) {
  ...
  if (actionItem.actionFunction) {
    ...
    actionItemContent = renderDOMSingle(
      typeof actionItem.actionDom === 'function' ? actionItem.actionDom() : actionItem.actionDom,
    )
    ...
    const actionItemContentTitle = actionItemContent.querySelector(
      getCssClassSelector(domConstants.VERTICAL_MENU_WRAPPER__WRAPPER_TITLE),
    )
    ...
    const actionItemMenu = renderMenu(actionItem.actionPopupMenu.menuItems, {
    ...
    throw new Error(
      'Action Item: no defined action; must have either actionFunction, actionDom, or actionPopupMenu',
    )
  }
