...
export function renderActionItem(actionItem) {
  ...
  const iconButton = actionItemElement.querySelector(
  ...
  if (actionItem.actionFunction) {
    ...
    const popupContentWrapper = /** @type {HTMLElement} */ (
      popupWrapper.querySelector(getCssClassSelector(domConstants.POPUP_CONTENT_WRAPPER))
    )
    if (!popupContentWrapper) {
      throw new Error('renderPopupMenu: contentWrapper not found')
    }
    popupContentWrapper.appendChild(
      renderDOMSingle(
