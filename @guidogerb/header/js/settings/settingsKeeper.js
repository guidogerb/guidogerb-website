...
function validateSettings(settingsToValidate) {
  if (!settingsToValidate.showTitle && !settingsToValidate.logo) {
    throw new Error(
      'validateSettings: A title must be shown if there is no logo. Please change the `showTitle` setting to be `true` or provide a logo image.',
    )
  }
}
...
class SettingsKeeper {
   ...
   * setGgpHeaderSettings() does a merge of the settings, this puts settings back to blank
   * starting settings
   */
  clearSettings() {
    this.settings = { ...defaultSettings }
