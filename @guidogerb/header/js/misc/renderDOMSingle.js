...
function renderDOM(str) {
  ...
  let result
  if (isString(str)) {
    const rendered = /** @type {Document} */ (
      isString(str) ? domParser.parseFromString(str, 'text/html') : str
    )

    const maybeResult =
      rendered.body.children.length > 1 ? rendered.body.children : rendered.body.children.item(0)
    if (!maybeResult) {
...
export function renderDOMSingle(str) {
  ...
  const firstChild = /** @type {HTMLElement | null} */ (
    dom instanceof HTMLCollection ? dom[0] : dom
  )
  if (!firstChild) {
    // eslint-disable-next-line no-console
    console.error(str)
