...
/*
  ...
  * do the same test as above but with the  ID button after logging in so that it opens the ggp Id menu
  ...
  function performPopup(delayMS) {
    ...
    if (!options?.isPerformPopup || options?.isPerformPopup()) {
      delayPopupTimeoutId = window.setTimeout(() => {
        ...
        // hide all tooltips
        document
          .querySelectorAll(getCssClassSelector(domConstants.TOOLTIP__WRAPPER))
