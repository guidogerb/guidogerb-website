...
async function validateTasksFile(filePath) {
  ...
  const headerIndex = lines.findIndex((line) => {
    ...
    return lower.includes('|') && lower.includes('name') && lower.includes('createddate') && !lower.includes('---');
  });

  if (headerIndex === -1) {
    errors.push(`${formatLocation(filePath)} - Missing markdown table header with required task fields.`);
    return;
  }
  ...
  const expectedColumns = ['name', 'createddate', 'lastupdateddate', 'completeddate', 'status', 'description'];
  ...
  for (let lineIndex = headerIndex + 2; lineIndex < lines.length; lineIndex += 1) {
    ...
    if (!ALLOWED_STATUS.has(normalizedStatus)) {
      errors.push(
        `${formatLocation(filePath, lineIndex + 1)} - status must be one of ${Array.from(ALLOWED_STATUS)
          .map((value) => `\"${value}\"`)
          .join(', ')} (found \"${status}\").`
      );
    }
  ...
  if (dataRowCount === 0) {
    errors.push(`${formatLocation(filePath, headerIndex + 1)} - Task table must include at least one task row.`);
  }
}
