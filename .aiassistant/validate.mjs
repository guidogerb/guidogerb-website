...
for (const rule of cfg.rules) {
    ...
    if (rule.mustNotIncludeAny && !notAny(rule.mustNotIncludeAny)) {
        fails.push({ rule: rule.id, severity: rule.severity, msg: rule.description + " (forbidden term present)" })
        continue
    }
    // mustIncludeAll
    if (rule.mustIncludeAll && !hasAll(rule.mustIncludeAll)) {
        fails.push({ rule: rule.id, severity: rule.severity, msg: rule.description + " (missing required terms)" })
        continue
    }
    // mustIncludeAny
    if (rule.mustIncludeAny && !hasAny(rule.mustIncludeAny)) {
        if (rule.severity !== "error") {
            fails.push({ rule: rule.id, severity: rule.severity, msg: rule.description + " (consider mentioning)" })
        } else {
            fails.push({ rule: rule.id, severity: rule.severity, msg: rule.description + " (no acceptable term found)" })
        }
    }
